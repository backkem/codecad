...
use kurbo::{BezPath, PathEl, Point, Affine};
use printpdf::*;

use crate::{Document, Shape, Color};
...
impl Default for PdfOptions {
    fn default() -> Self {
        Self {
            ...
            margin_mm: 10.0,
            skip_layers: [
                "E_DEBUG", "E_RAY_HIT", "E_RAY_EP", "E_RAY_MISS",
                "E_RAY_DOOR", "E_DOORS", "E_RAW", "E_VIS", "E_WALLS",
                "Defpoints",
...
pub fn export_pdf(doc: &Document, opts: &PdfOptions) -> Vec<u8> {
    let skip: std::collections::HashSet<&str> = opts.skip_layers.iter()
        .map(|s| s.as_str()).collect();

    let expanded = expand_entities(doc);

    let visible: Vec<&ExpandedEntity> = expanded.iter()
        .filter(|e| !skip.contains(e.layer.as_str()))
        .filter(|e| {
            doc.layers.iter().any(|l| l.name == e.layer && l.visible)
                || e.layer.starts_with("E_")
        })
        .collect();
    ...
    let to_pdf = |p: Point| -> printpdf::Point {
        ...
        printpdf::Point { x: mm_to_pt(x_mm), y: mm_to_pt(y_mm) }
    };
...
fn set_stroke(ops: &mut Vec<Op>, r: f32, g: f32, b: f32, lw: f32) {
    ops.push(Op::SetOutlineColor {
        col: printpdf::Color::Rgb(Rgb { r, g, b, icc_profile: None }),
    });
...
fn emit_path(ops: &mut Vec<Op>, path: &BezPath, to_pdf: &dyn Fn(Point) -> printpdf::Point) {
    ...
    for el in path.iter() {
        match el {
            PathEl::MoveTo(p) => {
                ...
                points.push(LinePoint { p: to_pdf(p), bezier: false });
            }
            PathEl::LineTo(p) => {
                points.push(LinePoint { p: to_pdf(p), bezier: false });
            }
            PathEl::CurveTo(p1, p2, p3) => {
                points.push(LinePoint { p: to_pdf(p1), bezier: true });
                points.push(LinePoint { p: to_pdf(p2), bezier: true });
                points.push(LinePoint { p: to_pdf(p3), bezier: false });
            }
            PathEl::QuadTo(p1, p2) => {
                ...
                points.push(LinePoint { p: cp1, bezier: true });
                points.push(LinePoint { p: cp2, bezier: true });
                points.push(LinePoint { p: pp2, bezier: false });
            }
            PathEl::ClosePath => {
                if let Some(first) = points.first() {
                    points.push(LinePoint { p: first.p, bezier: false });
                }
            }
        }
    }
    ...
    for seg in segments {
        ...
        ops.push(Op::DrawLine {
            line: Line { points: seg, is_closed: false },
        });
    }
}

fn resolve_color(ent: &ExpandedEntity, layer_color: &dyn Fn(&str) -> (f32, f32, f32)) -> (f32, f32, f32) {
    let (r, g, b) = if ent.color != Color::WHITE && ent.color != Color::rgb(0, 0, 0) {
        color_f32(ent.color)
    } else {
        layer_color(&ent.layer)
    };
    if r > 0.9 && g > 0.9 && b > 0.9 { (0.6, 0.6, 0.6) } else { (r, g, b) }
}

fn line_weight(layer: &str, base: f32) -> f32 {
    if layer.starts_with("E_") { base * 2.5 }
    else if layer.starts_with("S_WALL") { base * 1.5 }
    else { base }
}

fn update_bounds(shape: &Shape, min_x: &mut f64, min_y: &mut f64, max_x: &mut f64, max_y: &mut f64) {
    match shape {
        Shape::Text { position, height, text, .. } | Shape::MText { position, height, plain_text: text, .. } => {
        ...
        if let Shape::BlockInsert { block_name, position, rotation, x_scale, y_scale } = &ent.shape {
            if let Some(def) = doc.blocks.get(block_name) {
                ...
                for (shape, shape_layer, shape_color) in &def.shapes {
                    ...
                    let color = if *shape_color == Color::WHITE && ent.color != Color::WHITE { ent.color } else { *shape_color };
                    result.push(ExpandedEntity { shape: shape.transformed(xform), layer: layer.clone(), color });
                }
            }
        } else {
            result.push(ExpandedEntity { shape: ent.shape.clone(), layer: ent.layer.clone(), color: ent.color });
        }
    }
    result
}

fn color_f32(c: Color) -> (f32, f32, f32) { (c.r as f32 / 255.0, c.g as f32 / 255.0, c.b as f32 / 255.0) }

fn for_each_point(path: &BezPath, mut f: impl FnMut(Point)) {
    for el in path.iter() {
        match el {
            PathEl::MoveTo(p) | PathEl::LineTo(p) => f(p),
            PathEl::QuadTo(p1, p2) => { f(p1); f(p2); }
            PathEl::CurveTo(p1, p2, p3) => { f(p1); f(p2); f(p3); }
            PathEl::ClosePath => {}
        }
    }
}
